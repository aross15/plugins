 ...
 //                   dimensions
        ...
        createPlotMatrix: async function (position = null, dimensions = null) {
            ...
            try {
                ...
                multiVariateExtras.log(`Creating plot matrix with ${attributes.length} attributes: ${attributes.map(a => a.name).join(', ')}`);
         ...
         */
        createPairGraph: async function(attr1Name, attr1Type, attr2Name, attr2Type, position) {
            try {
                multiVariateExtras.log(`Creating graph for ${attr1Name} (${attr1Type}) vs ${attr2Name} (${attr2Type}) at position (${position.x}, ${position.y})`);

                // Create a graph using the original dataset
                const result = await connect.createGraph(
                    multiVariateExtras.datasetInfo.name,
                    attr1Name,
                    attr2Name,
                    null, // No legend for plot matrix
                    position,
